...
import { ErrorBanner, Loading, SuccessBanner } from "@/components/ui/Status";
...
export default function EventDetailsPage() {
  ...
  async function handlePrimaryAction() {
    ...
    try {
      ...
      const { data } = await api.post(
        `${process.env.NEXT_PUBLIC_API_URL}/participations/${event.id}/join`,
      );
      setActionSuccess(data.message);
    } catch (err) {
      setActionError(
        err.response?.data?.message || "Couldn't complete that action.",
      );
    } finally {
      setActing(false);
    }
  ...
  async function handleDelete() {
    ...
    try {
      await api.delete(`${process.env.NEXT_PUBLIC_API_URL}/events/${event.id}`);
      router.push(`${process.env.NEXT_PUBLIC_API_URL}/dashboard/my-events`);
    } catch (err) {
      setActionError(
        err.response?.data?.message || "Couldn't delete this event.",
      );
    }
  }
  ...
  async function submitReview(e) {
    ...
    try {
      await api.post(`${process.env.NEXT_PUBLIC_API_URL}/reviews`, {
        eventId: event.id,
        ...reviewDraft,
      });
      setReviewOpen(false);
    ...
    return (
      <>
        ...
        <div className="mx-auto max-w-3xl px-5 py-16 md:px-8">
          ...
          <Link
            href="/events"
            className="mt-4 inline-block text-sm font-medium text-ink hover:text-coral"
          >
            ← Back to events
          </Link>
        </div>
        ...
        <div className="ticket mt-6 flex flex-col md:flex-row">
          <div className="flex-1 p-6 md:p-8">
            ...
            <p className="mt-5 text-sm text-ink-soft">
              Hosted by{" "}
              <span className="font-medium text-ink">{event.owner?.name}</span>
            </p>
          </div>
        ...
        <section className="mt-12">
          ...
          {reviewOpen && (
            <form
              ...
              className="mt-4 rounded-ticket border border-ink/10 bg-white p-5"
            >
              ...
              <div className="mt-2 flex gap-1 text-xl">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setReviewDraft({ ...reviewDraft, rating: n })
                    }
                    className={
                      n <= reviewDraft.rating ? "text-gold" : "text-ink/15"
                    }
                  >
                    ★
                  </button>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setReviewDraft({ ...reviewDraft, comment: e.target.value })
                }
