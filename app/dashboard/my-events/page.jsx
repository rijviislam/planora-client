...
export default function MyEventsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Couldn't load your events. Please try again.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="mt-8">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {events.map((event) => (
              <div key={event.id} className="rounded-ticket border border-ink/10 bg-white p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-ink-soft">
                      ...
                      · {event.venue} · {Number(event.fee) === 0 ? "Free" : `৳${event.fee}`}
                    </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() =>
                        setParticipantsEventId(participantsEventId === event.id ? null : event.id)
                      }
                    ...
                    >
                      {participantsEventId === event.id ? "Hide participants" : "Manage participants"}
                    </button>
                    <button
